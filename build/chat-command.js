...
program2.command("remove").description("remove chat").action(() => chatAction.removeChat());
program2.command("change").description("change to another chat").action(() => chatAction.changeChat());
program2.command("prompt").description("prompt manager").option("-s, --select <name>", "select a prompt for the current chat").option("-m, --modify <prompt>", "modify the current chat's prompt").option("-p, --publish", "publish the current chat prompt").action((option) => {
  const optFun = {
...
program2.command("config").description("manage current chat configuration.").option("-c, --context-size <contextSize>", "update context size").option("-m, --model", `change chat model`).option("-w, --with-context", "change with-context", false).action(async (option) => {
  const optFun = {
...
program2.command("clear").description("clear the current chat message").action(() => chatAction.clearChatMessage());
program2.parseAsync();
