...
var textColor = (text) => println(text.replace(/(<[^>]+>)/g, (match) => tip(match)).replace(/(\[[^\]]+\])/g, (match) => important(match)));
...
program2.command("history").description("History Questions.").action(() => chatAction.printChatHistory());
...
program2.command("prompt").description("prompt manager").action(() => {
});
program2.command("config").description("Manage Current Chat Config.").option("-c, --context-size <contextSize>", "update context size.").option("-m, --model", `change chat model.`).option("-w, --with-context", "change with context", false).action(async (option) => {
  const optFun = {
