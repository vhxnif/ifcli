...
import type { LLMMessage, LLMParam, LLMResultChunk, LLMToolsCallParam } from '../types/llm-types'
import type MCPClient from '../types/mcp-client'
import type { ChatConfig, IChatStore, MessageContent } from '../types/store-types'
...
class AiRouterNode extends Node<AskShare> {
    ...
    override async post(shared: AskShare, prepRes: unknown, execRes: unknown): Promise<string | undefined> {
        const { tools } = shared
        if (tools) {
            return "toolsCall"
        }
...
class ToolsCallNode extends Node<AskShare> {
    ...
    override async post(shared: AskShare, prepRes: LLMToolsCallParam, execRes: LLMResultChunk): Promise<string | undefined> {
        shared.resultChunk = execRes
        await Promise.all(prepRes.mcps.map(it => it.close()))
        return undefined
    }

    override async execFallback(prepRes: LLMToolsCallParam, error: Error): Promise<void> {
        await Promise.all(prepRes.mcps.map(it => it.close()))
        throw error
    }
...
class StreamCallNode extends Node<AskShare> {
    ...
    override async post(shared: AskShare, prepRes: LLMParam, execRes: LLMResultChunk): Promise<string | undefined> {
        shared.resultChunk = execRes
        return undefined
    }
