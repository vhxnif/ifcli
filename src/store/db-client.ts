...
export class DBClient implements IDBClient {
	...
	chats(): Chat[] {
		return this.db.query(`SELECT ${this.chatColumn} FROM chat`).as(Chat).all();
	}
	...
	queryTopic(chatId: string): ChatTopic[] {
		return this.db
			.query(`SELECT ${this.chatTopicColumn} FROM chat_topic WHERE chat_id = ?`)
			.as(ChatTopic)
			.all(chatId);
	}
	...
	delMessage(topicId: string): void {
		this.db.prepare(`DELETE FROM chat_message WHERE topic_id = ?`).run(topicId);
	}
	...
	private exportMessageSql(p: { chatId?: boolean; topicId?: boolean }): string {
