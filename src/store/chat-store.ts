...
export class ChatStore implements IChatStore {
    ...
    queryChat = (name: string) => this.db.query(`SELECT ${this.chatColumn} FROM chat WHERE name = ?`).as(Chat).get(name)

    newChat = (name: string, prompt: string, model: string) => this.chatNotExistsRun(name, () => {
        const now = unixnow()
        const chatId = nanoid()
    ...
    removeChat = (name: string) => this.chatExistsRun(name, c => {
        const chatId = c.id
        this.db.transaction(() => {
    ...
    changeChat = (name: string) => this.chatExistsRun(name, c => {
        if (c.select) {
            return
        }
    ...
    currentChat = () => this.db.query(`SELECT ${this.chatColumn} FROM chat WHERE "select" = ?`).as(Chat).get(true)!

    saveMessage = (messages: MessageContent[]) => this.currentChatRun(c => {
        const statement = this.db.prepare(`INSERT INTO chat_message (id, chat_id, "role", content, pair_key, action_time) VALUES (?, ?, ?, ?, ?, ?)`)
        this.db.transaction(() => {
            messages.filter(it => !isEmpty(it.role) && !isEmpty(it.content))
    ...
    historyMessage = (count: number) => this.currentChatRun(c => this.queryMessage(c.id, count))

    modifySystemPrompt = (prompt: string) => this.currentChatConfigRun((_, cf) => this.db.prepare(`UPDATE chat_config SET sys_prompt = ?, update_time = ? where id = ?`).run(prompt, unixnow(), cf.id))

    modifyContextLimit = (contextLimit: number) => this.currentChatConfigRun((_, cf) => this.db.prepare(`UPDATE chat_config SET context_limit = ?, update_time = ? where id = ?`).run(contextLimit, unixnow(), cf.id))
    ...
    private currentChatConfigRun = <T>(f: (ct: Chat, cf: ChatConfig) => T): T => this.currentChatRun(c => f(c, this.queryChatConfig(c.id)))

    private queryMessage = (chatId: string, count: number) => this.db.query(`SELECT ${this.chatMessageColumn} FROM chat_message WHERE chat_id = ? Limit ?`).as(ChatMessage).all(chatId, count)

    private queryChatConfig = (id: string): ChatConfig => this.db.query(`SELECT ${this.chatConfigColumn} FROM chat_config WHERE chat_id = ?`).as(ChatConfig).get(id)!

    private deleteChatMessage = (chatId: string): void => { this.db.prepare(`DELETE FROM chat_message WHERE chat_id = ?`).run(chatId) }
}