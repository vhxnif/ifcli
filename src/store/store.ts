...
export class Store implements IStore {
	...
	private getChat(name?: string): ChatInfo {
		const chat = name ? this.client.queryChat(name) : this.client.currentChat();
		if (!chat) {
	...
	private configExt(chatId: string): ConfigExtAct {
		...
		return {
			...
			update: (ext) => this.client.updateConfigExt(chatId, JSON.stringify(ext)),
		} as ConfigExtAct;
	}
	...
	private topicMessage(): TopicMessageAct {
		return {
			...
			save: (messages: MessageContent[]) => this.client.saveMessage(messages),
		} as TopicMessageAct;
	}
	...
	get quickSwitch() {
		return {
			...
			saveOrUpdate: (k) => this.client.addOrUpdateCmdHis("chat_switch", k),
		} as QucikSwitchAct;
	}
