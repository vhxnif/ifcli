...
describe('Store - Simple Business Logic Tests', () => {
  ...
  describe('Quick switch operations', () => {
    ...
    test('should delegate add to DBClient', () => {
      ...
      // Assert
      expect(mockAddCmdHis).toHaveBeenCalledWith('chat_switch', 'test-key')
    })
    ...
    test('should delegate delete to DBClient', () => {
      ...
      // Assert
      expect(mockDelCmdHis).toHaveBeenCalledWith('chat_switch', 'test-key')
    })
    ...
    test('should delegate update to DBClient', () => {
      ...
      // Assert
      expect(mockUpdateCmdHis).toHaveBeenCalledWith('chat_switch', 'test-key', 5)
    })
    ...
    test('should delegate saveOrUpdate to DBClient', () => {
      ...
      // Assert
      expect(mockAddOrUpdateCmdHis).toHaveBeenCalledWith('chat_switch', 'test-key')
    })
  })
  ...
  describe('Cache operations', () => {
    ...
    test('should delegate set to DBClient', () => {
      ...
      // Assert
      expect(mockSaveOrUpdateCache).toHaveBeenCalledWith({ key: 'test', value: 'test-value' })
    })
  })
  ...
  describe('Prompt operations', () => {
    ...
    test('should delegate publish to DBClient', () => {
      ...
      // Assert
      expect(mockPublishPrompt).toHaveBeenCalledWith('test', '1.0', 'test content')
    })
  })
  ...
  describe('Export operations', () => {
    ...
    test('should delegate topic to DBClient', () => {
      ...
      // Assert
      expect(mockQueryChatTopicExportMessage).toHaveBeenCalledWith('chat-1', 'topic-1')
      expect(result).toHaveLength(1)
    })
  })
})