...
describe("DBClient - Database Operations Tests", () => {
	...
	describe("Message operations", () => {
		test("should save and query messages", () => {
			...
			const messages: MessageContent[] = [
				...
				createMessageContent(topicId, "assistant", "Hi there", "pair-1"),
			...
			expect(queriedMessages.some((m) => m.content === "Hello")).toBe(true);
			expect(queriedMessages.some((m) => m.content === "Hi there")).toBe(true);
			expect(queriedMessages.some((m) => m.content === "How are you?")).toBe(
				true,
			);
		});
		...
		test("should filter reasoning messages when withReasoning is false", () => {
			...
			const messages: MessageContent[] = [
				createMessageContent(topicId, "user", "Hello", "pair-1"),
				createMessageContent(topicId, "reasoning", "Thinking...", "pair-1"),
				createMessageContent(topicId, "assistant", "Hi there", "pair-1"),
			];
			...
			expect(queriedMessages.some((m) => m.role === "reasoning")).toBe(false);
		});
	});
	...
	describe("Transaction operations", () => {
		test("should execute transactions", () => {
			...
			const chats = dbClient.chats();
			expect(chats.some((chat) => chat.name === "transaction-chat")).toBe(true);
		});
	});
});
